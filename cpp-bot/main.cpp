...
i32 botsMoveEntityToTarget(i64 entity_id, f32 x, f32 y) EM_IMPORT(botsMoveEntityToTarget);
...
Vec start_position = Vec{};

EMSCRIPTEN_KEEPALIVE void step()
{
	if (step_count == 0) {
		const i32 game_state_buffer_capacity = 8 * 1024;
		game_state_buffer = Buffer {
			(i8*) malloc(game_state_buffer_capacity),
			0,
			game_state_buffer_capacity
		};
	}

	const GameState* game_state = GetGameState((void*) game_state_buffer.ptr);

	auto entities = game_state->entities();
	auto len = entities->size();
	for (auto i = 0; i < len; i++) {
		...
		if (entity->my()) {
			...
			botsMoveEntityToTarget(entity->id(), 0.0f, 0.0f);
		}
	}
